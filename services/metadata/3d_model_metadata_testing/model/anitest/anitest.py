...
def main():
    ...
    ## Still to do/test
        ...
        # Determine if animated or not, then say Yes or No
            # If so, then skip file steps?
            # Check how big the dump files are by saving them and looking at their file size - depending on this, choose to either get dumped object metadata or not
...
def unzip_and_extract_model_metadata(model_zip_file):
    ...
    for fn in filename_list:
        ...
        if file_metadata["ext"] == "zip":
            subzip_metadata = unzip_and_extract_model_metadata(fn)
